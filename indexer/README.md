...
|---|---|---|
| `JobEscrow` `0x5b19…66BC` | `JobOpened`, `JobDelivered`, `JobReleased`, `JobRefunded`, `JobDisputed`, `PasskeyRegistered` | `Job` (status, gasless or not, delivery latency, on time or not, release path: hirer / **verifier** / passkey / review window), `Hirer`, `AgentHirer` |
...
recent jobs from GraphQL. It refuses to publish if the indexer lags the head by more than 2,000
...
npm test                   # 7 handler tests (createTestIndexer, offline) + 3 Nansen-derivation tests
LIVE=1 ENVIO_MONAD_RPC_URL=http://localhost:8545 npm test
                           # + live: index Monad testnet from the deployment block to job #3 through
                           #   the proxy and compare agent 1908 with passportOf() at the same block
npx tsc --noEmit
```

The live test passed on 2026-09-23 (827 s through the proxy).
