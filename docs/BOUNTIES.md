...
| Real on-chain data | Monad testnet (10143) from the AgentPassport deployment block: both JobEscrow versions (v1 and v2, job ids scoped per escrow, v2 acceptance and cancels), AgentPassport and both canonical ERC-8004 registries. 13 agents and 7 jobs indexed (re-indexed from scratch 2026-09-23 at block 65,021,571). The live test indexed ~530k blocks and matched `passportOf(1908)` at the same block |
...
| Runs from the README | `docker compose up -d --build` → GraphQL on `:8088`. `node scripts/snapshot.ts` publishes the snapshot. Tests: 9 handler tests (incl. two escrows with the same job id, cancelled vs refunded) + 3 Nansen-derivation tests + the live comparison test |
